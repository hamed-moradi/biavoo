	    ...
	    BEGIN
	      ...
	      IF SUBSTRING(@value, 1, 7)='@object' 
	        INSERT INTO @hierarchy
	          ([Name], SequenceNo, ParentId, StringValue, ObjectId, ValueType)
	          SELECT @name, @SequenceNo, @ParentId, SUBSTRING(@value, 8, 5),
	            SUBSTRING(@value, 8, 5), 'object' 
	      ELSE 
	        IF SUBSTRING(@value, 1, 6)='@array' 
	          INSERT INTO @hierarchy
	            ([Name], SequenceNo, ParentId, StringValue, ObjectId, ValueType)
	            SELECT @name, @SequenceNo, @ParentId, SUBSTRING(@value, 7, 5),
	        ...
	        ELSE 
	          ...
	          ELSE 
	            IF @value IN ('true', 'false') 
	              INSERT INTO @hierarchy
	                ([Name], SequenceNo, ParentId, StringValue, ValueType)
	                SELECT @name, @SequenceNo, @ParentId, @value, 'boolean'
	            ELSE
	              IF @value='null' 
	                INSERT INTO @hierarchy
	                  ([Name], SequenceNo, ParentId, StringValue, ValueType)
	                  SELECT @name, @SequenceNo, @ParentId, @value, 'null'
	              ELSE
	                IF PATINDEX('%[^0-9]%', @value collate SQL_Latin1_General_CP850_Bin)>0 
	                  INSERT INTO @hierarchy
	                    ([Name], SequenceNo, ParentId, StringValue, ValueType)
	                    SELECT @name, @SequenceNo, @ParentId, @value, 'real'
	                ELSE
	                  INSERT INTO @hierarchy
	                    ([Name], SequenceNo, ParentId, StringValue, ValueType)
	                    SELECT @name, @SequenceNo, @ParentId, @value, 'int'
	      if @Contents=' ' Select @SequenceNo=0
	    END
	  END
	INSERT INTO @hierarchy ([Name], SequenceNo, ParentId, StringValue, ObjectId, ValueType)
	  SELECT '-',1, NULL, '', @ParentId-1, @type
	--
	   RETURN
	END